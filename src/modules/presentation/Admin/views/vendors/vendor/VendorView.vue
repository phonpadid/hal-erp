...
import { dataVendors } from "@/modules/shared/utils/data-vendors";
...
// State
const vendors = ref<VendorInterface[]>(dataVendors.value);
const loading = ref<boolean>(false);
const searchKeyword = ref<string>("");
const pagination = reactive({
  current: 1,
  pageSize: 10,
  total: dataVendors.value.length,
  totalPages: Math.ceil(dataVendors.value.length / 10),
});
...
const filteredVendors = computed(() => {
  if (!searchKeyword.value) return vendors.value;

  const keyword = searchKeyword.value.toLowerCase();
  return vendors.value.filter(
    (vendor) =>
      vendor.name.toLowerCase().includes(keyword) ||
      vendor.contact_info.toLowerCase().includes(keyword)
  );
});
...
const handleFormSubmit = async (formData: { name: string; contact_info: string }) => {
  try {
    ...
    if (isEditMode.value && selectedVendor.value) {
      ...
      // Simulate create
      const newVendor: VendorInterface = {
        id: (vendors.value.length + 1).toString(),
        ...formData,
...
const handleDeleteConfirm = async () => {
  ...
  try {
    submitLoading.value = true;
    // Simulate delete
    vendors.value = vendors.value.filter((v) => v.id !== selectedVendor.value?.id);
