...
onMounted(() => {
  if (props.category) {
    form.value.name = props.category.getName();
  }
});
...
const validateName = async () => {
  ...
  if (!isEdit.value || (props.category && form.value.name !== props.category.getName())) {
    const exists = await props.nameExists(form.value.name);
    if (exists) {
      nameError.value = `Category name '${form.value.name}' already exists`;
      return false;
    }
  }
