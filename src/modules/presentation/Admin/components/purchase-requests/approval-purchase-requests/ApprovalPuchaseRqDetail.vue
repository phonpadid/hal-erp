...
const customButtons = computed(() => {
  ...
  // Add Create PO button if PR is fully approved and PO hasn't been created yet
  if (shouldShowCreatePoButton.value) {
    baseButtons.splice(2, 0, {
      label: t("purchase-rq.btn.create_po"),
      icon: "mdi:file-document-plus",
      class: "bg-blue-500 flex items-center gap-2 text-white hover:bg-blue-600 mr-4",
  ...
  if (!currentStep) return baseButtons;
...
// Check if PR is fully approved (all steps completed with status_id === 2)
const isFullyApproved = computed(() => {
  const steps = requestDetail.value?.getUserApproval()?.approval_step ?? [];
  return steps.length > 0 && steps.every((step) => step.status_id === 2);
});

// Check if PO has been created
const isPoCreated = computed(() => {
  return requestDetail.value?.getIsCreatedPo() ?? false;
});

// Show Create PO button only when PR is fully approved but PO hasn't been created yet
const shouldShowCreatePoButton = computed(() => {
  return isFullyApproved.value && !isPoCreated.value;
});

// Handle Create PO button click
const handleCreatePO = () => {
  if (!requestDetail.value?.getId()) {
    error("ເກີດຂໍ້ຜິດພາດ", "ບໍ່ພົບເລກທີເອກະສານ PR");
    return;
  }
...
const handleApprove = async () => {
  ...
  if (currentStep.is_otp === true) {
    ...
    // ✅ เปิด modal เพื่อแสดง preview ลายเซ็นเหมือนกัน (แต่ไม่ต้องใส่ OTP)
    isOtpModalVisible.value = true;
  }
};
...
const handleOtpConfirm = async (otpCode: string) => {
  ...
  // ✅ ถ้า is_otp: false ไม่ต้องเช็ค otpCode (จะเป็น empty string)
  if (currentStep.is_otp !== false && !otpCode) {
  ...
  // ✅ ตรวจสอบ approval_id เฉพาะกรณีที่มี OTP เท่านั้น
  if (currentStep.is_otp && !approvalIdFromOtp) {
  ...
  try {
    ...
    const payload: SubmitApprovalStepInterface = {
      ...
      is_otp: currentStep.is_otp ?? false,
      otp: otpCode,
    };

    // ✅ เพิ่ม approval_id เฉพาะกรณีที่มี OTP
    if (currentStep.is_otp && approvalIdFromOtp) {
      payload.approval_id = approvalIdFromOtp;
    }
    ...
    if (success) {
      ...
      if (updatedPR) {
        requestDetail.value = updatedPR;
      }

      // ✅ Check if PR is now fully approved after this approval
      const steps = requestDetail.value?.getUserApproval()?.approval_step ?? [];
      const allApproved = steps.length > 0 && steps.every((step) => step.status_id === 2);
      const hasPO = requestDetail.value?.getIsCreatedPo() ?? false;

      // ✅ ກວດສອບວ່າເປັນຂັ້ນຕອນທຳອິດ (step 0) ຫຼືບໍ່
      if (currentStep.step_number === 0) {
        // Show success modal then redirect to PR list
      ...
      } else if (allApproved && !hasPO) {
        // ✅ Last step approved and no PO created - stay on page and show Create PO button
        isSuccessModalVisible.value = true;
        // Don't redirect - let user see the Create PO button
      } else if (allApproved && hasPO) {
        // ✅ Last step approved and PO already created - redirect to PR list
        isSuccessModalVisible.value = true;
        setTimeout(() => {
          router.push({ name: "apv_purchase_request.index" });
        }, 1500);
      } else {
        // ✅ Middle steps (step 2, 3, etc.) - show success modal
        isSuccessModalVisible.value = true;
      }
    }
  } catch (err) {
  ...
  <div class="container mx-auto py-1 px-0">
    ...
    >
      ...
      <header-component
        ...
        :document-number="requestDetail?.getPrNumber() || 'no data'"
    ...
    <OtpModal
      ...
      :is_otp="currentApprovalStep?.is_otp"
