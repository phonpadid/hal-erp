...
const handleConfirmSignature = async (remark?: string) => {
  ...
  try {
    ...
    const totalQuantity = poData.value.getPurchaseOrderItem().reduce((sum, item) => sum + item.getQuantity(), 0);
      ...
      <div class="user-info">
        ...
        <div class="table-section">
          ...
          <div class="total-section">
              ...
              <span class="total-value">{{ dataInfo.items.reduce((sum, item) => sum + item.getQuantity(), 0) }}</span>
            </div>
            <div class="total-item">
              <span class="total-label">ຈຳນວນລາຄາລວມ:</span>
              <span class="total-value">{{ formatPrice(dataInfo.total) }}</span>
            </div>
