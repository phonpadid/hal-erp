import type { VendorVankAccountInterface } from "@/modules/interfaces/vendors/vendor_bank_accounts/vendor-bank-accounts.interface";
import { ref } from "vue";

export const dataVendorBankAccounts = ref<VendorVankAccountInterface[]>([
  {
    id: "1",
    vendor_id: "bbb",
    currency_id: "LAK",
    bank_name: "ທະນາຄານ ABC",
    account_number: "1234567890",
    account_name: "ທະນາຄານ ABC ສາຂາ 1",
    is_selected: true,
    created_at: "2025-05-10 09:30:00",
    updated_at: "2025-05-10 09:30:00",
    deleted_at: null,
  },
  {
    id: "2",
    vendor_id: "ccc",
    currency_id: "LAK",
    bank_name: "ທະນາຄານ XYZ",
    account_number: "0987654321",
    account_name: "ທະນາຄານ XYZ ສາຂາ 2",
    is_selected: false,
    created_at: "2025-05-10 10:15:00",
    updated_at: "2025-05-18 14:22:00",
    deleted_at: null,
  },
]);
